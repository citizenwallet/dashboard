...
import { searchAccountAddressAction } from '@/app/[alias]/(dashboard)/members/action';
  ...
  useEffect(() => {
    const checkAddress = async () => {
      ...
      if (isValid) {
        ...
        const isAvailable = await searchAccountAddressAction({
          config: config,
          address: query
        });
  ...
  return (
    <div className="relative ml-auto flex-1 md:grow-0">
      ...
      {isvaildAddress && !isAvailableAddress && (
        <Button
          className="absolute right-0  h-7 m-1.5 w-fit font-normal text-xs"
          onClick={() => {
      ...
      {isvaildAddress && isAvailableAddress && (
        <Button
          ...
          onClick={() => {
            router.push(
              `members/${searchParams.get('query')?.toString()}/edit`
            );
          }}
        >
          Edit member
